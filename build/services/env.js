...
let EnvService = class EnvService {
    ...
    async create(payload) {
        ...
        const tab = new env_1.Env(Object.assign(Object.assign({}, payload), { position: position / 2 }));
    ...
    async set_envs() {
        const envs = await this.envs('', { position: -1 }, { status: { $ne: env_1.EnvStatus.disabled }, name: { $exists: true } });
        const groups = lodash_1.default.groupBy(envs, 'name');
        let env_string = '';
        for (const key in groups) {
            if (Object.prototype.hasOwnProperty.call(groups, key)) {
                const group = groups[key];
                env_string += `export ${key}="${lodash_1.default.map(group, 'value')
                    .join('&')
                    .replace(/ /g, '')}"\n`;
            }
        }
